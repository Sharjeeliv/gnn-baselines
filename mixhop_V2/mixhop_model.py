...
def sparse_dropout(x, drop_prob, num_entries, is_training):
  ...
  random_tensor += tf.random.uniform([num_entries])
...
def psum_output_layer(x, num_classes, use_softmax=True):
  ...
  if use_softmax:
    sum_q_weights = tf.compat.v1.get_variable(
      'psum_q', shape=[num_segments], initializer=tf.compat.v1.zeros_initializer, dtype=tf.float32, trainable=True)
    tf.compat.v1.losses.add_loss(tf.reduce_mean(input_tensor=(sum_q_weights ** 2)) * 1e-3 )
...
def mixhop_layer(x, sparse_adjacency, adjacency_powers, dim_per_power,
  ...
  for p, dim in zip(adjacency_powers, dim_per_power):
    ...
    with tf.compat.v1.variable_scope('l%i_p%s' % (layer_id, str(p))):
