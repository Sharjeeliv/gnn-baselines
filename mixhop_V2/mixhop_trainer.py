...
# Dataset Flags.
flags.DEFINE_string('dataset_name', 'ind.GSM2230757', '')
...
def main(unused_argv):
  ...
  y = tf.compat.v1.placeholder(tf.float32, [None, dataset.ally.shape[1]], name='y')
  ph_indices = tf.compat.v1.placeholder(tf.int64, [None])
  ...
  if FLAGS.architecture:
    ...
    model.add_layer('tf.compat.v1', 'sparse_tensor_to_dense')
    ...
    for j, dim in enumerate(layer_dims):
      ...
      if j != len(layer_dims) - 1:
        if FLAGS._batch_normalization:
          model.add_layer('tf.contrib.layers', 'batch_normalization')
  ...
  learn_rate = tf.compat.v1.placeholder(tf.float32, [], 'learn_rate')

  label_loss = tf.reduce_mean(input_tensor=tf.nn.softmax_cross_entropy_with_logits(
          labels=tf.stop_gradient(y), logits=sliced_output))
  tf.compat.v1.losses.add_loss(label_loss)
  loss = tf.compat.v1.losses.get_total_loss()
  
  if FLAGS.optimizer == 'MomentumOptimizer':
    optimizer = tf.compat.v1.train.MomentumOptimizer(lr, 0.7, use_nesterov=True)
  else:
    optimizer_class = getattr(tf.compat.v1.train, FLAGS.optimizer)
  ...
  print("Number of parameters: ", 
    numpy.sum([numpy.prod(v.get_shape().as_list()) for v in tf.compat.v1.trainable_variables()]))
  print({v.name: v.get_shape().as_list() for v in tf.compat.v1.trainable_variables()})
