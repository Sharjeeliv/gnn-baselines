...
from ._layers import GraphAttentionLayer
import operator
keras = tf.keras
...
class H2GCN(tf.keras.Model):
    def __init__(self, layer_setups,
                 sparse_input=True, use_attention=False, l2_regularize_weight=0):
        super().__init__()
        self.layer_objs = []
        self.dropout_inds = []
        self.attention_inds = []
        ...
        for ind, (layerType, layerConf) in enumerate(layer_setups):
            ...
            elif layerType == Layer.ATTENTION:  # Add attention layer option
                self.attention_inds.append(len(self.layer_objs))  # Track attention layers
                self.layer_objs.append(layers.GraphAttentionLayer(**layerConf))  # Assume GraphAttentionLayer is defined in _layers
    ...
    def call(self, adj, inputs, adjhops, training=False, returnBefore=0, executeAfter=0,
        ...
        for ind, layer in enumerate(self.layer_objs):
            ...
            elif ind in self.attention_inds:  # Apply attention layer
                inputs = layer(adj, inputs, adjhops)
