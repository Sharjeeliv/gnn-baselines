...
def compute_edge_weights(features):
    # Compute cosine similarity between nodes using the feature matrix
    print(len(features))
...
class PlanetoidData:
    ...
    def load_data(self, dataset_str, dataset_path="data", save_plot=None, val_size=None):
        ...
        edge_weights = compute_edge_weights(features.toarray())
        adj = adj.multiply(edge_weights)  # Apply the edge weights to the adjacency matrix
...
def normalize_adj(adj):
    ...
    # Use weighted adjacency for normalization
    return adj.dot(d_mat_inv_sqrt).transpose().dot(d_mat_inv_sqrt).tocoo()


def preprocess_adj(adj):
    """Preprocessing of adjacency matrix for simple GCN model and conversion to tuple representation."""
    adj_weighted = adj + sp.eye(adj.shape[0])  # Add self-loops if needed
    adj_normalized = normalize_adj(adj_weighted)
