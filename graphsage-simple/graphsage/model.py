...
def accuracy(output, labels):
    preds = output.cpu().detach().numpy().argmax(1)
...
def run_graphsage(feat_data, labels, adj_lists, train, val, test, num_classes,
    ...
    for batch in range(args.epochs):
        ...
        loss = graphsage.loss(batch_nodes, 
                Variable(torch.cuda.LongTensor(labels[np.array(batch_nodes)])))
        loss.backward()
        optimizer.step()
        end_time = time.time()
...
if __name__ == "__main__":
    ...
    parser.add_argument("--dataset", default="ind.GSM2230757", type=str)
    ...
    data = dataset.PlanetoidData(args.dataset, args._dataset_path, val_size=args.val_size)

    run_graphsage(data.features.toarray(), data.labels[:, np.newaxis], data.dos_graph, 
        np.where(data.train_mask)[0], np.where(data.val_mask)[0], np.where(data.test_mask)[0], data.num_labels,
